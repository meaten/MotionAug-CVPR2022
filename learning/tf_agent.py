...
class TFAgent(RLAgent):
    ...
    def save_model(self, out_path):
        with self.sess.as_default(), self.graph.as_default():
            if self.tf_writer == None:
                self.tf_writer = tf.summary.FileWriter(os.path.join("log/", self.output_dir))
            try:
                ...
                summary = tf.Summary(value=[tf.Summary.Value(tag='test_reward_per_frame', simple_value=self.avg_test_return_mean)])
                self.tf_writer.add_summary(summary, self._total_sample_count)
                summary = tf.Summary(value=[tf.Summary.Value(tag='test_reward_per_episode', simple_value=self.avg_test_return)])
                self.tf_writer.add_summary(summary, self._total_sample_count)
            except NameError:
                Logger.print("Failed to save model")
        return

    def load_model(self, in_path):
        with self.sess.as_default(), self.graph.as_default():
            #variables = self._get_saver_vars()
            #variables_to_restore = [v for v in variables if 'g_norm' not in v.name]
            #saver =  tf.train.Saver(variables_to_restore)
            #saver.restore(self.sess, in_path)

            """
            from tensorflow.python import pywrap_tensorflow
            reader = pywrap_tensorflow.NewCheckpointReader(in_path)
            restore_dict = dict()
            ...
            for var in variables:
                if var.name.split(':')[0] not in var_to_shape_map:
                    print(var.name)
