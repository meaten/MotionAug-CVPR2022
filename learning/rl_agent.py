...
class RLAgent(ABC):
    ...
    def __init__(self, world, id, json_data):
        ...
        self.test_return_mean = 0.0
        self.avg_test_return = 0.0
        self.avg_test_return_mean = 0.0
    ...
    def _update_test_return(self, path):
        ...
        path_reward_mean = path.calc_hacked_normalized_return()
        self.test_return += path_reward
        self.test_return_mean += path_reward_mean
    ...
    def _update_mode_test(self):
        if (self.test_episode_count * MPIUtil.get_num_procs() >= self.test_episodes):
            ...
            global_return_mean = MPIUtil.reduce_sum(self.test_return_mean)
            avg_return_mean = global_return_mean / global_count
            self.avg_test_return_mean = avg_return_mean
    ...
    def _store_path(self, path):
        ...
        if valid_path:
            self.train_return = path.calc_return_mean()
        ...
        if (self.replay_buffer_initialized):  
            if (self._valid_train_step()):
                ...
                for i in range(iters):
                    ...
                    self.logger.log_tabular("Test_Return", self.avg_test_return_mean)
