...
std::unique_ptr<cDrawMesh> cDrawUtil::gOctoSphereMesh = nullptr;
std::unique_ptr<cDrawMesh> cDrawUtil::gSphereWireSimpleMesh = nullptr;
std::unique_ptr<cDrawMesh> cDrawUtil::gOctoSphereWireSimpleMesh = nullptr;
...
void cDrawUtil::Drawoctosphere(double d, double h, double y, eDrawMode draw_mode)
{
	cDrawUtil::PushMatrixView();
	cDrawUtil::Scale(tVector(d, h, y, 1));
	int elem = gOctoSphereMesh->GetNumVerts() / 2;

	if (draw_mode == eDrawSolid)
	{
		gOctoSphereMesh->Draw(GL_TRIANGLES, 0, elem);
	}
	else if (draw_mode == eDrawWire)
	{
		gOctoSphereMesh->Draw(GL_LINES, 0, elem);
	}
	else if (draw_mode == eDrawWireSimple)
	{
		gOctoSphereWireSimpleMesh->Draw(GL_LINES, 0, ((gNumStacks/2) * 4 + gNumSlice) *2);
	}
	else
	{
		assert(false); // unsupported draw mode
...
void cDrawUtil::BuildMeshes()
{
	...
	cMeshUtil::BuildOctoSphereMesh(gNumStacks, gNumSlice, gOctoSphereMesh);
	cMeshUtil::BuildSphereWireSimpleMesh(gNumStacks, gNumSlice, gSphereWireSimpleMesh);
	cMeshUtil::BuildOctoSphereWireSimpleMesh(gNumStacks, gNumSlice, gOctoSphereWireSimpleMesh);
