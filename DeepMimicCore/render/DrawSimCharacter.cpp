...
void cDrawSimCharacter::DrawInfoRewardLog(const cCircularBuffer<double>& rlog, const cCircularBuffer<double>& rlog_imitate,const cCircularBuffer<double>& rlog_goal, const cCamera& cam, const double origin_y)
{
	const double min_val = 0;
	const double max_val = 1;

	int num_val = static_cast<int>(rlog.GetSize());
	double aspect = cam.GetAspectRatio();
	...
	tVector origin = tVector::Zero();
	origin[0] = 1 - w * 1.05;
	origin[1] = origin_y - h * 1.05;
	origin[2] = -1;

	int capacity = static_cast<int>(rlog.GetCapacity());
	...
	if (num_val > 0)
	{
		...
		prev_val = rlog_imitate[0];
		for (int i = 1; i < num_val; ++i)
		{
			curr_val = rlog_imitate[i];
		...
		prev_val = rlog_goal[0];
		for (int i = 1; i < num_val; ++i)
		{
			curr_val = rlog_goal[i];
