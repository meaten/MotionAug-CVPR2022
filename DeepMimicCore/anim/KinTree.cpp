...
void cKinTree::LerpPoses(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& pose0, const Eigen::VectorXd& pose1, double lerp, Eigen::VectorXd& out_pose)
{
	...
	for (int j = 1; j < num_joints; ++j)
	{
		...
		if (joint_type == eJointTypeSpherical)
		{
			...
			//std::cout << rot0.norm() << std::endl << pose0.segment(offset, size) << std::endl;
			//std::cout << rot1.norm() << std::endl << pose1.segment(offset, size) << std::endl;
			//lerpposes fails before PostProcessPose, so normalize quat
			if (std::abs(rot0.norm() - 1) > 0.000001 || std::abs(rot1.norm() - 1) > 0.000001)
			{
				rot0.normalize(); rot1.normalize();
			}
