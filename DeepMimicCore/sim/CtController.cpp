...
void cCtController::RecordState(Eigen::VectorXd& out_state)
{
	...
	if(mPhaseRepNum != int(mTime / mCyclePeriod))
	{
		mPhaseRepNum = int(mTime / mCyclePeriod);
	}
...
void cCtController::BuildStateVel(Eigen::VectorXd& out_vel) const
{
	...
	for (int i = 0; i < num_parts; ++i)
	{
		...
		//printf("%f\n%f\n%f\n%f\n\n\n\n",mTime, curr_vel[0], curr_vel[1], curr_vel[2]);sleep(0);
		if (mRecordWorldRootRot && i == root_id)
		{
			if (flip_stance)
			{
...
void cCtController::SetKinChar(std::shared_ptr<cKinCharacter> kin_char)
{
