...
bool cCtrlBuilder::BuildController(const tCtrlParams& params, std::shared_ptr<cCtController>& out_ctrl)
{
	bool succ = true;
	...
	case eCharCtrlCtPD:
		...
		break;
	case eCharCtrlCtPDRes:
		succ = BuildCtPDResController(params, out_ctrl);
		break;
	case eCharCtrlCtVel:
...
bool cCtrlBuilder::BuildCtController(const tCtrlParams& params, std::shared_ptr<cCtController>& out_ctrl)
{
	bool succ = true;
...
bool cCtrlBuilder::BuildCtPDController(const tCtrlParams& params, std::shared_ptr<cCtController>& out_ctrl)
{
	bool succ = true;
...
bool cCtrlBuilder::BuildCtPDResController(const tCtrlParams& params, std::shared_ptr<cCtController>& out_ctrl)
{
	bool succ = true;
	std::shared_ptr<cCtPDResController> ctrl = std::shared_ptr<cCtPDResController>(new cCtPDResController());
	ctrl->SetGravity(params.mGravity);
	ctrl->Init(params.mChar.get(), params.mCtrlFile);
...
bool cCtrlBuilder::BuildCtVelController(const tCtrlParams& params, std::shared_ptr<cCtController>& out_ctrl)
{
	bool succ = true;
