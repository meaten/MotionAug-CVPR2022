...
void cImpPDController::CalcControlForces(double time_step, Eigen::VectorXd& out_tau)
{
	...
	// SetTargetVel is never used. tar_vel = [0, ... ,0]
	//std::cerr << tar_vel << std::endl;
	//std::cerr << mKinChar->GetPhase() << std::endl << std::endl;
	//const double mTime = mKinChar->GetTime();
	//Eigen::VectorXd tar_pose;
	//mKinChar->CalcPose(mTime+t, tar_pose);
	//mKinChar->CalcVel(mTime+t, tar_vel);
...
void cImpPDController::SetKinChar(std::shared_ptr<cKinCharacter> kin_char)
{
	mKinChar = kin_char.get();
}