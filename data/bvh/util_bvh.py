...
def FK(mocap, idx, joint_name):
    assert(joint_name in mocap.get_joints_names())

    offsets = []
    rotations = []
    
    indicator, joint_name = mocap.get_joint(joint_name).value
    while(indicator != 'ROOT'):
        offsets.append(getOffset(mocap, joint_name))
        rotations.append(getRotation(mocap, idx, joint_name))
        indicator, joint_name = mocap.joint_parent(joint_name).value
    else:
        offset_root = getTranslation_root(mocap, idx, joint_name)
        rotation_root = getRotation(mocap, idx, joint_name)
...
def FK_all(mocap, idx):
    offset_dict = {}
    rotaton_dict = {}
    for joint_name in mocap.get_joints_names():
        indicator = mocap.get_joint(joint_name).value[0]
        if (indicator == 'ROOT'):
            offset_root = getTranslation_root(mocap, idx, joint_name)
            rotation_root = getRotation(mocap, idx, joint_name)
        else:
            offset_dict[joint_name] = getOffset(mocap, joint_name)
            rotaton_dict[joint_name] = getRotation(mocap, idx, joint_name)
    ...
    for joint_name in mocap.get_joints_names():
        indicator = mocap.get_joint(joint_name).value[0]
        if (indicator == 'ROOT'):
            pos_dict[joint_name] = offset_root 
            accum_rot_dict[joint_name] = rotation_root
        else:
            indicator, parent_name = mocap.joint_parent(joint_name).value
            pos_dict[joint_name] = pos_dict[parent_name] + accum_rot_dict[parent_name].apply(offset_dict[joint_name])
            accum_rot_dict[joint_name] = accum_rot_dict[parent_name] * rotaton_dict[joint_name]
...
def getRotation(mocap, idx, joint_name):

    order, channels = getRotationOrderAndChannels(mocap, joint_name)
    euler_angle = mocap.frame_joint_channels(idx, joint_name, channels)
...
def getTranslation_root(mocap, idx, joint_name='Hips'):
    channels = ['Xposition', 'Yposition', 'Zposition']
    translation = mocap.frame_joint_channels(idx, joint_name, channels)
