...
class H36motion(Dataset):
    ...
    def __init__(self, path_to_data, actions, input_n=10, output_n=10, dct_n=20, split=0, sample_rate=2, data_mean=0,
        ...
        all_seqs = all_seqs[:, :, dim_used]  # seq_id, frame, dim
        all_seqs = all_seqs.transpose(0, 2, 1)  # seq_id, dim, frame
        all_seqs = all_seqs.reshape(-1, input_n + output_n)  # seq_id * dim, frame
        all_seqs = all_seqs.transpose()  # frame, seq_id * dim
