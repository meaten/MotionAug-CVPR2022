...
def load_data_cmu_3d(path_to_dataset, actions, input_n, output_n, data_std=0, data_mean=0, is_test=False):
    ...
    for action_idx in np.arange(nactions):
        ...
        for examp_index in np.arange(count):
            ...
            action_sequence = readCSVasFloat(filename)
            n, d = action_sequence.shape
            exptmps = Variable(torch.from_numpy(
                action_sequence)).float().cuda()
...
def expmap2xyz_torch_hdm05(expmap):
    parent, offset, rotInd, expmapInd = forward_kinematics._some_variables_hdm05()
    xyz = forward_kinematics.fkl_torch(
        expmap, parent, offset, rotInd, expmapInd)
    return xyz
        ...
        for action_idx in np.arange(len(actions)):
            ...
            if not (subj == 5):
                ...
                if len(sampled_seq) == 0:
                    ...
                    sampled_seq = np.concatenate(
                        (sampled_seq, seq_sel2), axis=0)
                    complete_seq = the_sequence1
                    complete_seq = np.append(
                        complete_seq, the_sequence2, axis=0)
        ...
        for action_idx in np.arange(len(actions)):
            ...
            if not (subj == 5):
                for subact in [1, 2]:  # subactions
                    ...
                    the_sequence = np.array(action_sequence[even_list, :])
                    the_seq = Variable(torch.from_numpy(
                ...
                the_sequence1 = np.array(action_sequence[even_list, :])
                the_seq1 = Variable(torch.from_numpy(
                    the_sequence1)).float().cuda()
                the_seq1[:, 0:6] = 0
                ...
                the_sequence2 = np.array(action_sequence[even_list, :])
                the_seq2 = Variable(torch.from_numpy(
                    the_sequence2)).float().cuda()
                the_seq2[:, 0:6] = 0
                ...
                fs_sel1, fs_sel2 = find_indices_srnn(
                    num_frames1, num_frames2, seq_len)
                seq_sel1 = the_sequence1[fs_sel1, :]
                seq_sel2 = the_sequence2[fs_sel2, :]
                if len(sampled_seq) == 0:
                    sampled_seq = seq_sel1
                    ...
                    sampled_seq = np.concatenate(
                        (sampled_seq, seq_sel1), axis=0)
                    sampled_seq = np.concatenate(
                        (sampled_seq, seq_sel2), axis=0)
                    complete_seq = np.append(
                        complete_seq, the_sequence1, axis=0)
                    complete_seq = np.append(
                        complete_seq, the_sequence2, axis=0)
...
def load_data_hdm05(paths_to_dataset, subjects, actions, sample_rate, input_n, output_n, data_mean=0, data_std=0, is_test=False):
    ...
    if not is_test:
        complete_seq = np.concatenate(sampled_seq, axis=0)
        data_std = np.std(complete_seq, axis=0)
        data_mean = np.mean(complete_seq, axis=0)
    ...
    return sampled_seq, np.asarray(idxs), dimensions_to_ignore, dimensions_to_use, data_mean, data_std


def define_actions_hdm05(action):
    """
    Define the list of actions we are using.
    ...
    actions = actions = ['cartwheel', 'hit', 'elbow',
                         'deposit', 'grab', 'throw',
                         'kick', 'punch', 'hop', 'turn',
