...
from utils.hdm05_motion import HDM05motion
...
def main(opt):
    ...
    model = nnmodel.GCN(input_feature=(input_n + output_n), hidden_feature=opt.linear_size, p_dropout=opt.dropout,
                        num_stage=opt.num_stage, node_n=45)
    if is_cuda:
        model.cuda()
    ...
    test_data = dict()
    for act in acts:
        test_dataset = HDM05motion(path_to_data=opt.data_dir_hdm05, actions=act, input_n=input_n, output_n=output_n, split=1,
