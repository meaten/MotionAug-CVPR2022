...
def bvhToMimic(bvhfile, output_path, **kwargs):
    ...
    motion_name = bvhToMotion(bvh, bvhfile, path_data, output_path, settings)
    return motion_name
...
def bvhToMotion(bvh, bvhfile, path, output_path, settings, return_string=False):
    ...
    bvh_name = os.path.splitext(os.path.basename(bvhfile))[0]
    if output_path == "":
        with open(os.path.join(path, f"bvh/motion_{bvh_name}.txt"), "w") as f:
            f.write(json.dumps(outputDict, indent=4, separators=(',', ': ')))
    else:
        with open(os.path.join(output_path, f"motion_{bvh_name}.txt"), "w") as f:
            f.write(json.dumps(outputDict, indent=4, separators=(',', ': ')))

    return f"motion_{bvh_name}.txt"
